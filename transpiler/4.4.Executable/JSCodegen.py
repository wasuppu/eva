...
class JSCodegen:
    ...
    # ThrowStatement.
    def ThrowStatement(self, exp):
        return f"throw {self.gen(exp['argument'])};"

    # AwaitExpression.
    def AwaitExpression(self, exp):
        return f"await {self.gen(exp['argument'])}"

    # ThisExpression.
    def ThisExpression(self, exp):
        return 'this'

    # TryStatement.
    def TryStatement(self, exp):
        return f"try {self.gen(exp['block'])} {self.gen(exp['handler'])}"

    # CatchClause.
    def CatchClause(self, exp):
        return f"catch ({self.gen(exp['param'])}) {self.gen(exp['body'])}"

    # ObjectExpression.
    def ObjectExpression(self, exp):
        properties = list(map(lambda prop: self.gen(prop), exp['properties']))
        return '{' + f"{', '.join(properties)}" + '}'

    # ObjectProperty.
    def ObjectProperty(self, exp):
        return f"{self.gen(exp['key'])}: {self.gen(exp['value'])}"

    # ArrayExpression.
    def ArrayExpression(self, exp):
        elements = list(map(lambda element: self.gen(element), exp['elements']))
        return f"[{', '.join(elements)}]"

    # MemberExpression.
    def MemberExpression(self, exp):
        print(exp)
        if exp.get('computed'):
            return f"{self.gen(exp['object'])}[{self.gen(exp['property'])}]"
        return f"{self.gen(exp['object'])}.{self.gen(exp['property'])}"
    ...
    def YieldExpression(self, exp):
        ...
        argument = ''
        if exp['argument'] != None:
            argument = f" {self.gen(exp['argument'])}"
