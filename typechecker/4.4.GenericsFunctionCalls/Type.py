...
class Union(Type):
    def __init__(self, name, optionTypes):
        super().__init__(name)
        self.optionTypes = optionTypes

    # Whether this union includes all types:
    def includesAll(self, types):
        if len(types) != len(self.optionTypes):
            return False
        for type in types:
            if not self.equals(type):
                return False
        return True
    ...
    def equals(self, other):
        ...
        # Other union:
        if isinstance(other, Union):
            return self.includesAll(other.optionTypes)

        # Anything else:
        for optionType in self.optionTypes:
            if optionType.equals(other):
                return True
        return False
...
class GenericFunction(Type):
    def __init__(self, name, genericTypesStr, params, body, returnType, env):
        if name == None:
            name = 'lambda'
        name = f"{name} <{genericTypesStr}>"
        super().__init__(name)
        self.genericTypes = genericTypesStr.split(',')
        self.params = params
        self.returnType = returnType
        self.body = body
        self.env = env
