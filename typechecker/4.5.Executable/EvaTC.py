...
class EvaTC:
    ...
    # Self-evaluating:
        ...
        if exp[0] == 'def':
            ...
            if not self._isGenericDefFunction(exp):
                name = exp[1]
                params = exp[2]
                returnTypeStr = exp[4]
        ...
        if exp[0] == 'lambda':
            # 1. Generic function:
            if self._isGenericLambdaFunction(exp):
                return self._createGenericFunctionType(exp, env)
            # 2. Simple function:
            return self._createSimpleFunctionType(exp, env)
        ...
        if isinstance(exp, list):
            ...
            if isinstance(fn, Type.GenericFunction):
                ...
                actualTypes = self._extractActualCallTypes(exp)

                # Map the generic types to the actual types:
                genericTypesMap = self._getGeneticTypesMap(fn.genericTypes, actualTypes)

                # Bind parameters and return types:
                boundParams, boundReturnType = self._bindFunctionTypes(fn.params, fn.returnType, genericTypesMap)
                ...
                actualFn = self._tcFunction(boundParams, boundReturnType, fn.body, fn.env)
            ...
            # Passed arguments:
            argTypes = list(map(lambda arg: self.tc(arg, env), argValues))
            return self._checkFunctionCall(actualFn, argTypes, env, exp)

        raise Exception(f"Unknown type for expression {exp}.")

    # Maps generic parameter types to actual types.
    def _getGeneticTypesMap(self, genericTypes, actualType):
        boundTypes = {}
        for i in range(len(genericTypes)):
            boundTypes[genericTypes[i]] = actualType[i]
        return boundTypes

    # Binds generic parameters and return type to actual types.
    def _bindFunctionTypes(self, params, returnType, genericTypesMap):
        actualParams = []

        # 1. Bind parameter types:
        for i in range (0, len(params)):
            paramName, paramType = params[i]

            actualParamType = paramType

            # Generic type -> rewrite to actual.
            if paramType in genericTypesMap:
                actualParamType = genericTypesMap[paramType]

            actualParams.append([paramName, actualParamType])

        # 2. Bind return type:
        actualReturnType = returnType
        if returnType in genericTypesMap:
            actualReturnType = genericTypesMap[returnType]

        return actualParams, actualReturnType

    # Extracts types for generic parameter types.
    # (combine <string> "hello")
    def _extractActualCallTypes(self, exp):
        matched = re.search(r'^<([^>]+)>$', exp[1])
        if not matched:
            raise Exception(f"No actual types provided in generic call: {exp}")
        data = matched.groups()
        return data[0].split(',')

    # Simple function declarations (no generic parameters).
    # Such functions are type-checked during declaration time.
    def _createSimpleFunctionType(self, exp, env):
    ...
    def _createGenericFunctionType(self, exp, env):
        _tag, genericTypes, params, _retDel, returnType, body = exp
        return Type.GenericFunction(None, genericTypes[1:-1], params, body, returnType, env)

    # Whether the funtion is generic.
    # (def foo <K> ((x K)) -> K (+ x x))
    def _isGenericLambdaFunction(self, exp):
        return len(exp) == 6 and re.search(r'^<[^>]+>$', exp[1])

    # Whether the funtion is generic.
    # (def foo <K> ((x K)) -> K (+ x x))
    def _isGenericDefFunction(self, exp):
        return len(exp) == 7 and re.search(r'^<[^>]+>$', exp[2])

    # Transforms def to var-lambda:
    def _transformDefToVarLambda(self, exp):
        # 1. Generic function:
        if self._isGenericDefFunction(exp):
            _tag, name, genericTypeStr, params, _retDel, returnTypeStr, body = exp
