...
class Sexp:
    ...
    def _parseAtom(self):
        ...
        if self._cursor < len(self._expression):
            matched = False
            for spec in specs:
                ...
                if match != None:
                    matched = True
                    _, end = match.span()
                    atom = self._expression[self._cursor:self._cursor+end]
                    self._cursor += end
                    if isNumber(atom):
                        atom = toNumber(atom)
                    return atom
            if not matched:
                token = self._expression[self._cursor:].split(' ')[0].rstrip(')')
                raise Exception('Not an atom ' + token)
